...
token: generateToken(user._id),
  user: { _id: user._id, name: user.name, email: user.email, role: user.role, profilePic: user.profilePic, bio: user.bio }
});
...
router.put('/profile', protect, upload.single('profilePic'), async (req, res) => {
  try {
    console.log('Profile update request:', {
      body: req.body,
      file: req.file ? { filename: req.file.filename, size: req.file.size, originalname: req.file.originalname } : null,
      userId: req.user._id
    });

    const user = await User.findById(req.user._id);
    console.log('Current user before update:', { name: user.name, profilePic: user.profilePic });
    ...
    console.log('Updated user after save:', { name: updated.name, profilePic: updated.profilePic });
    res.json(updated);
  } catch (err) {
